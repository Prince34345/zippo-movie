...
function MovieDetail({ loading }) {
  ...
  const GetResponse = async () => {
    try {
      let res = await Detail("tv", id);
      //  console.log("res",res)
      setResponse(res);
      let cred = await axios.get(`https://api.themoviedb.org/3/tv/${id}/credits?api_key=bf4c074c437b81e1700a800748c358e0`)
      setCreds(cred.data)
    } catch (error) {
            ...
            <div className="max-[1000px]:flex justify-center items-center flex-col">
              ...
              <div className="flex max-[1180px]:flex-col  w-1/2 max ">
                {Response?.genres?.map((curElem) => {
                  return <p className="text-white px-5 py-2 max-xl:px-3 rounded-lg  max-xl:py-1 text-center flex justify-center items-center max-xl:bg-transparent   bg-theme mx-5 font-Josefin-Sans font-black     ">{curElem.name}</p>;
                })}
              </div>
              <div className="flex m-5 justify-between  w-2/3 font-semibold font-Sansita max-[1000px]:w-1/2">
                <div className="text-slate-400 bg-opacity-30  ">{Response?.seasons?.[0]?.episode_count} Episodes </div>
